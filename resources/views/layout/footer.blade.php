<link rel="stylesheet" href="/css/footer.css">
<footer class="site-footer">
<!-- Site footer -->
    <div class="container mt-2">
        <div class="container">
            <div class="row">
                <div class="col-sm-12 col-md-6 col-xl-5 ms-4 me-4">
                    <h6>ProffessionAble</h6>
                    <p class="text-justify">ProfessionAble adalah jaringan ekosistem talenta sosial yang menghubungkan penyandang disabilitas dengan pekerja inklusi di Indonesia. Misi kami adalah memudahkan para penyandang disabilitas pencari kerja untuk terus mengembangkan karirnya dan menemukan peluang kerja yang tepat untuk mereka.</p>
                </div>
                <div class="col-md-3 col-xl-3 ms-4 me-3">
                    <h6>Fitur</h6>
                    <ul class="footer-links">
                        <li><a href="" class="text-decoration-none">Profil</a></li>
                        <li><a href="" class="text-decoration-none">Lowongan Cocok</a></li>
                        <li><a href="" class="text-decoration-none">Lowongan sesuai minat</a></li>
                        <li><a href="" class="text-decoration-none">Lowongan sesuai aplikasi</a></li>
                        <li><a href="" class="text-decoration-none">Pengajuan kerja langsung</a></li>
                        <li><a href="" class="text-decoration-none">Lamaran dikirim</a></li>
                    </ul>
                </div>
                <div class="col-md-3 col-xl-3">
                    <h6>Perusahaan</h6>
                    <ul class="footer-links">
                        <li><a href="" class="text-decoration-none">Tentang</a></li>
                        <li><a href="" class="text-decoration-none">Media</a></li>
                        <li><a href="" class="text-decoration-none">Mitra</a></li>
                        <li><a href="" class="text-decoration-none">Hubungi Kami</a></li>
                        <li><a href="" class="text-decoration-none">Kebijakan Privasi</a></li>
                        <li><a href="" class="text-decoration-none">FAQ</a></li>
                    </ul>
                </div>
            </div>
            <hr>
        </div>
        <div class="container">
            <div class="row">
                <div class="col-md-8 col-sm-6 col-xs-12">
                    <p class="copyright-text">Copyright &copy; 2023 All Rights Reserved by
                        <a href="#">ProffessionAble</a>.
                    </p>
                </div>
                <div class="col-md-4 col-sm-6 col-xs-12">
                    <ul class="hubungi-Kami">
                        <li><a class="facebook" href="#"><i class="fa fa-facebook"></i></a></li>
                        <li><a class="twitter" href="#"><i class="fa fa-twitter"></i></a></li>
                        <li><a class="dribbble" href="#"><i class="fa fa-dribbble"></i></a></li>
                        <li><a class="linkedin" href="#"><i class="fa fa-linkedin"></i></a></li>
                    </ul>
                </div>
            </div>
        </div>
    </div>
</footer>